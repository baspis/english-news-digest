...
def render_edition_page(
    ...
    for record in edition.article_records:
        ...
        if record.selection_reason.startswith("jt_comments_"):
            ...
            rank_comments = f'<span class="meta">{html.escape(n)} comments at selection</span>'
    ...
    <header class="panel page-header">
      ...
      <p class="meta">{edition.actual_article_count} articles · {html.escape(category_summary)}{html.escape(comments_meta)} · Generated {html.escape(generated)}</p>
      <p class="lede">Japan Today articles published on this date (JST): one per category (National, Crime, Business, Politics, World, Tech), chosen by reader comment count within each category.</p>
    </header>
    <section class="panel">
